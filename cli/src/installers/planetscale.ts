...
export const planetscaleInstaller: Installer = ({ projectDir }) => {
  ...
  const schemaSrc = path.join(extrasDir, "src/server/db/drizzle", `with-mysql.ts`)
  const schemaDest = path.join(projectDir, "src/server/db/schema.ts")

  const jstackSrc = path.join(extrasDir, "src/server/jstack", `drizzle-with-planetscale.ts`)

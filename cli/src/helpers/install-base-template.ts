...
export const installBaseTemplate = async ({ projectName, projectDir, pkgManager, noInstall }: InstallerOptions) => {
  ...
  if (fs.existsSync(projectDir)) {
    if (fs.readdirSync(projectDir).length === 0) {
      ...
      if (overwriteDir === "clear") {
        spinner.info(`Emptying ${chalk.cyan.bold(projectName)} and creating JStack app..\n`)
